<?php include 'CommonFile/All_Css_Files.php'; ?>
<?php include 'CommonFile/Header.php'; ?>

<link rel="stylesheet" href="Assets/css/DigitalStartups.css">
<link rel="stylesheet" href="Assets/css/ContentWriting.css">

<section>
    <div class="innerMainBannerBg">
        <div class="container">
            <div class="col-lg-6 m-auto text-center mainBannerHeading mt-5 pt-4 pb-4">
                <img class="w-100" src="Assets/images/ServicesContent/ServicesContentWritingText-02.png" alt="">
                <h1 style="bottom: 25px;"><strong>Content</strong> Writing</h1>
            </div>
        </div>
    </div>
</section>

<section>
    <div class="DigitalStartupsSetionTwoBg">
        <div class="container">
            <div class="row">
                <div class="col-lg-4 m-auto" data-aos="fade-right">
                    <img class=" m-auto d-block DigitalStartupsSetionImg01" style="width: 100%!important;" src="Assets/images/ServicesContent/ServicesContentWritingIcon-02.png" alt="">
                </div>
                <div class="col-lg-7 m-auto" data-aos="fade-left">
                    <div class="smartWork">
                        <h2>
                            <strong>
                                <i>
                                    Unique Services For Digital Startup
                                </i>
                            </strong>
                        </h2>
                        <p>
                            <strong>
                                <i>
                                    And best practices to always remains
                                    your first choice.
                                </i>
                            </strong>
                        </p>
                        <div class="col-lg-4">
                            <a class="ReadArticle mt-2" href="contact-us.php">Get Started</a>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </div>
</section>

<section class="DigitalStartupsSetionThreeBg">
    <div class="container">
        <div class="row">
            <div class="col-lg-5 m-auto" data-aos="fade-right">
                <div class="ServicesList mb-4">
                    <div class="row">
                        <div class="col-6 col-lg-5 m-auto">
                            <ul>
                                <li>
                                    Lorem ipsum
                                </li>
                                <li>
                                    Lorem ipsum
                                </li>
                                <li>
                                    Lorem ipsum
                                </li>
                                <li>
                                    Lorem ipsum
                                </li>
                                <li>
                                    Lorem ipsum
                                </li>
                            </ul>
                        </div>
                        <div class="col-6 col-lg-5 m-auto">
                            <ul>
                                <li>
                                    Lorem ipsum
                                </li>
                                <li>
                                    Lorem ipsum
                                </li>
                                <li>
                                    Lorem ipsum
                                </li>
                                <li>
                                    Lorem ipsum
                                </li>
                                <li>
                                    Lorem ipsum
                                </li>
                            </ul>
                        </div>
                    </div>
                </div>
            </div>
            <div class="col-lg-6 m-auto" data-aos="fade-left">
                <div class="ContentHeading">
                    <h2>We have Develope Easy Ways</h2>
                    <p>
                        Lorem ipsum dolor sit amet, consectetur adipiscing elit. Donec sem nulla, semper sed ullamcorper vel, tempus non quam. Duis nisl ex, euismod non nibh eu, ultrices congue nisl. Duis hendrerit pulvinar laoreet. Praesent pretium augue vitae nibh dignissim posuere. Proin sed porttitor ligula. Morbi in diam feugiat, varius velit id, vulputate arcu. Nam sed eros vulputate, finibus justo eu, faucibus nunc. Nulla facilisi. Vivamus nisl metus, placerat a elementum vel, convallis euismod erat.
                    </p>
                </div>
            </div>
        </div>
    </div>
</section>

<section class="DigitalStartupsSetionFourBg">
    <div class="col-lg-4 text-center m-auto">
        <h2 class=" text-light">Our Process</h2>
    </div>
    <div class="container">
        <div class="row">
            <div class="col-lg-1"></div>
            <div class="col-lg-2 text-center">
                <div class="Type">
                    <img class="processImg" src="Assets/images/Services/LogoDesign/img01.png" alt="">
                    <h2 style="font-weight: 600;font-size: 1.6rem;">Brief</h2>
                </div>
            </div>
            <div class="col-lg-2 text-center">
                <div class="Type">
                    <img class="processImg" src="Assets/images/ServicesContent/OurProcessImg1-02.png" alt="">
                    <h2 style="font-weight: 600;font-size: 1.6rem;">Brainstorming</h2>
                </div>
            </div>
            <div class="col-lg-2 text-center">
                <div class="Type">
                    <img class="processImg" src="Assets/images/ServicesContent/OurProcessImg2-02.png" alt="">
                    <h2 style="font-weight: 600;font-size: 1.6rem;">Keyword Research</h2>
                </div>
            </div>
            <div class="col-lg-2 text-center">
                <div class="Type">
                    <img class="processImg" src="Assets/images/ServicesContent/OurProcessImg3-02.png" alt="">
                    <h2 style="font-weight: 600;font-size: 1.6rem;">Writing</h2>
                </div>
            </div>
            <div class="col-lg-2 text-center">
                <div class="Type">
                    <img class="processImg" src="Assets/images/Services/LogoDesign/img05.png" alt="">
                    <h2 style="font-weight: 600;font-size: 1.6rem;">Delivered</h2>
                </div>
            </div>
            <div class="col-lg-1"></div>
        </div>
        <div class="col-lg-2 m-auto mt-5">
            <a class="ReadArticle mt-2" href="contact-us.php">Get Started</a>
        </div>
    </div>
</section>

<section class="servicesContentOne">
    <div class="container">
        <div class="row">
            <img class="MainSectionImg06" src="Assets/images/HomePage/MainSectionImg06.png" alt="">
            <div class="col-lg-4"></div>
            <div class="col-lg-6 m-auto" data-aos="fade-left">
                <h2>
                    <i>Web Content</i>
                </h2>
                <p>
                    <i>
                        Lorem ipsum dolor sit amet, consectetur adipiscing elit. Donec sem nulla, semper sed ullamcorper vel, tempus non quam. Duis nisl ex, euismod non nibh eu, ultrices congue nisl. Duis hendrerit pulvinar laoreet. Praesent pretium augue vitae nibh dignissim posuere. Proin sed porttitor ligula. Morbi in diam feugiat, varius velit id, vulputate arcu. Nam sed eros vulputate, finibus justo eu, faucibus nunc. Nulla facilisi. Vivamus nisl metus, placerat a elementum vel, convallis euismod erat.
                    </i>
                </p>

                <div class="col-lg-2 mt-3">
                    <a class="ReadArticle mt-2" href="contact-us.php">Get Started</a>
                </div>
            </div>
        </div>
    </div>
</section>

<section class="servicesContentTwo">
    <div class="container">
        <div class="row">
            <div class="col-lg-6 m-auto" data-aos="fade-right">
                <h2>
                    <i>Blogs Writing</i>
                </h2>
                <p>
                    <i>
                        Lorem ipsum dolor sit amet, consectetur adipiscing elit. Donec sem nulla, semper sed ullamcorper vel, tempus non quam. Duis nisl ex, euismod non nibh eu, ultrices congue nisl. Duis hendrerit pulvinar laoreet. Praesent pretium augue vitae nibh dignissim posuere. Proin sed porttitor ligula. Morbi in diam feugiat, varius velit id, vulputate arcu. Nam sed eros vulputate, finibus justo eu, faucibus nunc. Nulla facilisi. Vivamus nisl metus, placerat a elementum vel, convallis euismod erat.
                    </i>
                </p>

                <div class="col-lg-2 mt-3">
                    <a class="ReadArticle mt-2" href="contact-us.php">Get Started</a>
                </div>
            </div>
            <div class="col-lg-4"></div>
            <img class="MainSectionImg07" src="Assets/images/HomePage/MainSectionImg06.png" alt="">
        </div>
    </div>
</section>

<section class="servicesContentThree">
    <div class="container">
        <div class="row">
            <img class="MainSectionImg06" src="Assets/images/HomePage/MainSectionImg06.png" alt="">
            <div class="col-lg-4"></div>
            <div class="col-lg-6 m-auto" data-aos="fade-left">
                <h2>
                    <i>Articles Writing</i>
                </h2>
                <p>
                    <i>
                        Lorem ipsum dolor sit amet, consectetur adipiscing elit. Donec sem nulla, semper sed ullamcorper vel, tempus non quam. Duis nisl ex, euismod non nibh eu, ultrices congue nisl. Duis hendrerit pulvinar laoreet. Praesent pretium augue vitae nibh dignissim posuere. Proin sed porttitor ligula. Morbi in diam feugiat, varius velit id, vulputate arcu. Nam sed eros vulputate, finibus justo eu, faucibus nunc. Nulla facilisi. Vivamus nisl metus, placerat a elementum vel, convallis euismod erat.
                    </i>
                </p>

                <div class="col-lg-2 mt-3">
                    <a class="ReadArticle mt-2" href="contact-us.php">Get Started</a>
                </div>
            </div>
        </div>
    </div>
</section>

<section class="DigitalStartupsSetionNineBg">
    <div class="container">
        <div class="col-lg-4 m-auto text-center mt-3 mb-5">
            <h2><strong>Packages</strong></h2>
        </div>
        <div class="row">
            <div class="col-lg-4">
                <div class="card cardRounded">
                    <div class="card-title">
                        Blog Writing
                    </div>
                    <ul>
                        <li>100% Original Content</li>
                        <li>Topics As Per Request</li>
                        <li>SEO Friendly</li>
                        <li>72 Hours Turnaround Time</li>
                        <li>Copyrights</li>
                        <li>100% Plagiarism-Free</li>
                        <li>Unlimited Revisions</li>
                        <li>Proofing & Editing By Experts</li>
                    </ul>
                    <div class="pricingBox">
                        <h2><Span>$</Span> 15 <span>USD</span></h2>
                    </div>
                </div>
            </div>
            <div class="col-lg-4">
                <div class="card cardRounded">
                    <div class="card-title">
                        Article Writing
                    </div>
                    <ul>
                        <li>Engaging, Product-Focused Content</li>
                        <li>72 Hours Turnaround Time</li>
                        <li>Professional Marketing Campaigns</li>
                        <li>100% Plagiarism-Free</li>
                        <li>Unlimited Revisions</li>
                        <li>Written On Specific Keywords</li>
                        <li>Proofing & Editing By Experts</li>
                    </ul>
                    <!-- <div class="col-lg-4 m-auto mt-3">
                        <a class="ReadArticle mt-2" href="contact-us.php">Get Started</a>
                    </div> -->
                    <div class="pricingBox">
                        <h2><Span>$</Span> 15 <span>USD</span></h2>
                    </div>
                </div>
            </div>
            <div class="col-lg-4">
                <div class="card cardRounded shadow-lg">
                    <div class="card-title">
                        Web Content
                    </div>
                    <ul>
                        <li>Product-Focused Content</li>
                        <li>72 Hours Turnaround Time</li>
                        <li>Marketing Campaigns</li>
                        <li>100% Plagiarism-Free</li>
                        <li>Unlimited Revisions</li>
                        <li>Copyrights</li>
                        <li>According To Exact Content</li>
                        <li>Proofing & Editing By Experts</li>
                    </ul>
                    <!-- <div class="col-lg-4 m-auto mt-3">
                        <a class="ReadArticle mt-2" href="contact-us.php">Get Started</a>
                    </div> -->
                    <div class="pricingBox">
                        <h2><Span>$</Span> 40 <span>USD</span></h2>
                    </div>
                </div>
            </div>
        </div>
    </div>
</section>

<section>
    <div class="HomePageSectionFour">
        <img class="MainSectionImg06" src="Assets/images/HomePage/MainSectionImg06.png" alt="">
        <div class="container">
            <div class="col-lg-6 m-auto text-center">
                <div class="OurCustomer">
                    <h2>Our <span>Customer’s</span></h2>
                </div>
            </div>
            <div class="col-lg-8 m-auto mt-3 mb-3">
                <div class="slider03">
                    <div>
                        <img src="Assets/images/SliderImages/ClientImg01.png" alt="">
                    </div>
                    <div>
                        <img src="Assets/images/SliderImages/ClientImg02.png" alt="">
                    </div>
                    <div>
                        <img src="Assets/images/SliderImages/ClientImg03.png" alt="">
                    </div>
                    <div>
                        <img src="Assets/images/SliderImages/ClientImg04.png" alt="">
                    </div>
                </div>
            </div>
        </div>
    </div>
</section>

<?php include 'CommonFile/ContactSection.php'; ?>

<?php include 'CommonFile/Footer.php'; ?>